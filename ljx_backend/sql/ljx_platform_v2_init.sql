-- ================================================================
-- 榫卯非遗文化传承平台 - 完整数据库初始化脚本 v3.1
-- --------------------------------------------------------------
-- 版本:       v3.1 (2026-07-01 整合版, 含关注/课程下载前置 SQL)
...
--   * 新增 3 条 course 种子数据, 对应商城 3 个课程类商品
--   * 修复后重新生成完整脚本 (恢复自 git 提交 166b8e5)
-- --------------------------------------------------------------
-- v3.0 变更 (2026-06-29):
...
CREATE TABLE `product` (
  ...
  `course_id`    INT            DEFAULT NULL COMMENT '关联课程ID (category_id=4 时有效)',
  ...
  KEY `idx_product_course`   (`course_id`),
...
-- ------------------------------------------------------------
-- 4.8 课程数据 (对应商城课程类商品, 用于"下载课程"功能)
-- ------------------------------------------------------------
INSERT INTO `course`
  (`category_id`, `user_id`, `title`, `description`, `cover_image`, `duration`, `difficulty`, `status`, `create_time`)
VALUES
  (4, @admin1_id, '榫卯入门: 30天从零到熟练', '视频课程共24节, 总时长8小时。从工具认知、画线、开榫到组装, 手把手教学。附赠图纸电子版。', '../images/入门.png', 480, 1, 1, NOW()),
  (4, @admin1_id, '明式家具复制实战', '高级课程共18节, 以王世襄旧藏明式圈椅为蓝本, 全程复刻教学含料单及1:1图纸。', '../images/进阶.png', 720, 2, 1, NOW()),
  (4, @admin1_id, '古建筑榫卯结构解析', '深度课程共12节, 系统讲解唐宋明清不同时期斗栱、柱头、梁架榫卯系统的演变与受力分析。', '../images/高级.png', 360, 3, 1, NOW());

-- 将商城课程类商品与 course 表关联 (product.course_id)
UPDATE `product` SET `course_id` = (SELECT `course_id` FROM `course` WHERE `title` = '榫卯入门: 30天从零到熟练') WHERE `product_name` = '榫卯入门: 30天从零到熟练';
UPDATE `product` SET `course_id` = (SELECT `course_id` FROM `course` WHERE `title` = '明式家具复制实战')           WHERE `product_name` = '明式家具复制实战';
UPDATE `product` SET `course_id` = (SELECT `course_id` FROM `course` WHERE `title` = '古建筑榫卯结构解析')         WHERE `product_name` = '古建筑榫卯结构解析';
